...
public class Worker extends Employee {
    ...
    public boolean seniorityIsLongerThanYears(short years) {
        if (years < 0) {
            throw new IllegalArgumentException("Number of years can not be negative");
        }

        return (short) Period.between(employmentDate, LocalDate.now()).getYears() > years;
    }

    public boolean seniorityIsLongerThanMonths(short months) {
        if (months < 0) {
            throw new IllegalArgumentException("Number of months can not be negative");
        }

        return (short) Period.between(employmentDate, LocalDate.now()).getMonths() > months;
    }
